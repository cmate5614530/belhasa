      ...
      <div style="width: 100%; margin: auto;margin-top: 60px;    font-weight: bold; font-size: 18px;" id="table">
        ...
        <div  v-if="arabic == 1" style="direction: rtl">
          ...
          <table class="table hea">
            ...
            <tr>
              <th style="font-size: initial;">
                <div v-if="test_final_result == 0" class="text-right" style="text-align: right">نأسف لقد رسبت فى الإختبار</div>
                <div v-if="test_final_result == 1" class="text-right" style="text-align: right">مبروك لقد نجحت فى الإختبار </div>
              </th>
            </tr>
            </tbody>
          </table>
        ...
        <b-card header="" header-tag="h4" class="bg-success-card datatable-material"><br><br>
          ...
          <div v-else>
            <div v-show="testResult.length">
              <div style="font-weight: 600; margin-bottom: 10px;">{{$t('student.labels.student_info')}}</div>
              <table>
                <tr>
                  <td>{{$t('print.student_id')}}</td>
                  <td>{{this.$store.state.user.studentId}}</td>
                </tr>
                <tr>
                  <td>{{$t('print.student_name')}}</td>
                  <td>{{this.$store.state.user.name}}</td>
                </tr>
                <tr>
                  <td>{{$t('student.labels.email')}}</td>
              ...
              <table>
                <tr>
                  <td style="text-align: right">{{$t('table.no_of_corrects')}}  : {{this.numberofCorrects}}</td>
                </tr>
                <tr>
                  <td style="text-align: right">{{$t('table.no_of_incorrects')}} :{{this.testConfig.noofquestion - this.numberofCorrects}}</td>
                </tr>
              </table>
            </div>
            <div class="no-result" v-show="testResult.length == 0">
              <div><li class="fa fa-warning">{{$t('home.alert.result')}}</li></div>
            </div>
          </div>
        </b-card>
      </div>
...
export default {
  ...
  created() {
    ...
    //  get testConfigData
    User_ApiService.get('testConfig',{category:this.$route.params.categoryId}).then(
        result=>{
    ...
    //end
    //getTestResult
    User_ApiService.post('getTestResult',{testID:this.$route.params.id}).then(
        result=>{
          let data=result.data;
          console.log('-----------',data)
    ...
    User_ApiService.post('getTestDetail',this.model).then(
        result=>{
          let data=result.data;
          this.loading = true
